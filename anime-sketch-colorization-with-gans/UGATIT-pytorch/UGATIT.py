...
class UGATIT(object) :
    ...
    def build_model(self):
        ...
        self.testA = ImageFolder(os.path.join(self.datapath, 'testA'), test_transform)
        self.testB = ImageFolder(os.path.join(self.datapath, 'testB'), test_transform)
    ...
    def test(self):
        ...
        if not len(model_list) == 0:
            ...
            path = os.path.join(self.result_dir, self.dataset, 'model')
            print(path)
            print(iter)
            self.load(path, iter)
            print(" [*] Load SUCCESS")
        ...
        for n, (real_A, _) in enumerate(self.testA_loader):
            ...
            fake_B = RGB2BGR(tensor2numpy(denorm(fake_A2B[0])))
            cv2.imwrite(os.path.join(self.output_dir, '{}_ugatit'.format(self.dataset), '%d_real_A.png' % (n + 1)), real_A * 255.0)
            cv2.imwrite(os.path.join(self.output_dir, '{}_ugatit'.format(self.dataset), '%d_fake_B.png' % (n + 1)), fake_B * 255.0)
            print("processed {} images.".format(n+1))
            if n == total:
                break
